...
# Your directory chart as a multi-line string
tree_text = """
...
def create_from_tree(text):
    path_stack = []
    
    for line in text.strip().split('\n'):
        # Remove tree symbols and comments (anything after #)
        clean_line = re.sub(r'[│├└─|]', ' ', line).split('#')[0].strip()
        if not clean_line: continue
        
        # Calculate depth based on original leading characters
        # (Simplified: check the number of characters before the first word)
        depth = line.find(line.lstrip()[0]) // 4 
        ...
        if clean_line.endswith('/') or '.' not in clean_line:
